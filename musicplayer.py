...
def playMusic():
    if(root.songAdded == False):
        root.screenMessage.set("First add some Music !")
    else:
        try:
...
one.pack(fill=X)
two = Label(root, text="To My First Music Player", bg="white",fg="blue")
two.pack(fill=X)
three = Label(root, text="Created By Akash", bg="green",fg="white")
three.pack(fill=X)
...
root.screenMessage.set("Welcome, to Akash Music Player")
